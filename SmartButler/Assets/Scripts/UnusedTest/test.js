...
if (tagName != "MovingSurface") {
 ...
 { if (grounded) { 
    ...
    moveDirection = controller.GetComponent.<Rigidbody>().velocity;
    ...
    //Controller Move  takes place here 
  		myCharacterController.Move (calculatedMovement);
...
function OnControllerColliderHit (hit : ControllerColliderHit) { 
    ...
    if (hit.moveDirection.y < -0.9 && hit.normal.y > 0.5) { 
        activePlatform = hit.collider.transform; tagName = hit.collider.tag; 
 } 
} 